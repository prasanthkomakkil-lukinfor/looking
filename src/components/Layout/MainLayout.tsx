...
export function MainLayout() {
  ...
  return (
   <div className="min-h-screen bg-gray-50 flex flex-col max-w-lg w-full mx-auto px-4">
      {/* Content */}
        ...
        <div className="max-w-lg mx-auto flex">
          {tabs.map(({ id, label, icon: Icon }) => (
            ...
            >
              <Icon
                size={22}
                className={activeTab === id ? 'text-teal-500' : 'text-gray-400'}
                strokeWidth={activeTab === id ? 2.5 : 1.8}
              />
              <span className={`text-xs font-medium ${
                activeTab === id ? 'text-teal-500' : 'text-gray-400'
              }`}>
                {label}
              </span>
              {activeTab === id && (
